...
- Un mensaje repetido cierra el movimiento activo con `status_alta = 3` y agrega el siguiente con el mismo `id_referencia`; un lead nuevo crea un hilo con `status_alta = 1`. La `fecha_estimada` se conserva desde el movimiento activo cuando existe y se asigna con `CURDATE()` cuando no existe una fecha previa.
